...
`ros2_ws/src/` also contains a fifth, non-package folder, `cart_pole_gz_train/` (no `package.xml`/`CMakeLists.txt`, so `colcon build` skips it) — it trains a Stable-Baselines3 PPO policy for the same robot headlessly via `gz.sim8.TestFixture`, driving the real `cart_joint`/`pole_joint` through the ECM directly, ported from the root `cart_pole/` project's approach rather than going over `rclpy`/`ros_gz_bridge` like `commander`. See `ros2_ws/src/CLAUDE.md` for details.
