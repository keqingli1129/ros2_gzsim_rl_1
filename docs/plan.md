...
>
> **Amendment 2 (post-implementation, from Task 7's implementation): the reset type validated here, `reset.model_only`, is not what production uses.** Direct measurement while implementing `run_inference.py` showed `reset.model_only` is a complete no-op against this world — after a disturbed episode, issuing it and polling position/velocity immediately afterward showed both continuing to evolve exactly along their pre-reset trajectory, no discontinuity at all. This task only ever checked that `JointStatePublisher` kept publishing, never that the reset changed any joint's position/velocity, so the no-op went undetected here. Production (`run_inference.py`) was switched to `reset.all` instead, confirmed to actually reset position/velocity to ~0 on this world without hitting the `JointStatePublisher`-killing teardown bug documented above for the unrelated `commander`/`robomaster_rale` world. This script's validation of the unsubscribe/resubscribe pattern (the first Amendment above) still holds for either reset type and was not re-run against `reset.all` specifically as a separate task — see Task 7's section (Amendment below) and `run_inference.py`'s `_reset_world` docstring for the full explanation.
>
> **Amendment 3 (final whole-branch review): this script itself was retargeted at `reset.all`.** Amendment 2 above flagged that this script's message-count-only assertions against `reset.model_only` were exactly the blind spot that let that reset type's no-op behavior go undetected — the model was at rest both before and after its reset, so a working reset and a no-op reset looked identical on that check alone. The code below (and the run instructions in Steps 1-3) is left as originally written for historical record, but `verify_reset_preserves_joint_state.py` as shipped no longer matches it: it now (a) applies a force burst to disturb `cart_joint`/`pole_joint` before resetting, so a no-op is distinguishable from a real one, (b) issues `reset.all` instead of `reset.model_only`, matching what `run_inference.py` actually ships with, and (c) asserts the post-reset position/velocity read back near zero, not just that `joint_state` keeps publishing. See the file itself for the current source of truth.
...
> **Amendment (post-implementation, from Task 7's implementation and the final whole-branch review): the code below does not match what shipped.** The code block, as originally written, uses `request.reset.model_only = True` in `_reset_world` — Task 6's Amendment 2 already explains why production doesn't: `reset.model_only` measured as a complete no-op on this world. The shipped `run_inference.py` uses `request.reset.all = True` instead, confirmed to work (see Task 6's Amendment 2 and Amendment 3).
>
> A second, later fix (from the final whole-branch review, after this task was first marked complete) is also not reflected below: `ApplyJointForce` has no `Reset()` and simply holds the last commanded force (confirmed via `nm -DC` on its `.so`), so the pre-fall ±30N force stayed latched through `reset.all` and kept driving the freshly-reset cart/pole — measured to cause runaway "0.00s, resetting world..." storms (dozens of consecutive resets, RPC `ok=True` every time). The main loop's reset branch now (a) zeroes `cmd_force` before unsubscribing/resetting, and (b) checks the postcondition against the *first* fresh post-reset `joint_state` reading instead of one taken after a fixed `time.sleep(0.5)` — the sleep previously gave the (then still-latched) force time to drive the system back out of bounds before the check ever ran, so a genuine runaway could pass as "reset succeeded."
>
> A third fix, found while re-verifying the second fix above with dedicated scratch repros (`repro_reset_settle2.py`, `repro_reset_retry.py` — checking the *first* fresh reading on every single forced reset, not just after an artificial delay, surfaces this): `reset.all` itself was measured to occasionally not take effect at all on its very first request — the post-reset reading comes back nearly identical to the pre-reset one, independent of the RPC's own `ok` flag (reproduced with both `ok=True` and `ok=False`). This isn't the already-documented "`ok` is an unreliable signal in the direction of false negatives" case — it's a genuine, physical no-op on the reset itself, roughly 1-in-3 to 2-in-5 in dedicated stress measurements. Retrying immediately was measured to reliably recover, always within 2 attempts across two separate 10-consecutive-reset stress runs. The reset branch now retries up to `MAX_RESET_ATTEMPTS` (5, a large margin over the observed max of 2) before raising, both in `run_inference.py`'s main loop and in Task 6's `verify_reset_preserves_joint_state.py` (which would otherwise itself flake on this same behavior).
>
> See `run_inference.py`'s own `_reset_world` docstring and its main loop for the current, correct code — this section is left as originally written for historical record of the plan as given, not updated in place.
